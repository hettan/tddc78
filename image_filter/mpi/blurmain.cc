...
int main(int argc, char** argv)
{
  ...
  int root=0; //main process

  const int work_size = size*10;
  
  double *work = new double [work_size*sizeof(double)];
  MPI::COMM_WORLD.Scatter( work, work_size/size, MPI::DOUBLE,
			   rcv, work_size/size, root);
